...
A REST API for managing a book collection â€?Flask, PostgreSQL, Docker.

**Live:** https://book-api-uttz.onrender.com â€?try [`/books`](https://book-api-uttz.onrender.com/books) or [`/health`](https://book-api-uttz.onrender.com/health)
...
deployment. It started on SQLite and was migrated to PostgreSQL â€?the notes below
...
|--------|-----------------|-------------------------------|---------|----------|
| GET    | `/`             | Service and endpoint listing  | 200     | â€?       |
| GET    | `/health`       | Liveness + database check     | 200     | 503      |
| GET    | `/books`        | List books, `?author=` filter | 200     | â€?       |
...
cheap â€?opening one per request was fine. A PostgreSQL connection costs a
...
rather than an `INSERT` followed by a `SELECT` â€?and there is no window where
...
own database URL. `.env` is gitignored â€?connection strings contain credentials.
